...
PWD = pwd;
data_path = '/fileserver/fetal/Arvind/fMRI/DHCP/DHCP/sub-CC00379XX17_ses-120400/proposed/';
cd(data_path);
...
%fmri_fname_mask  = 
fmri_fname = '/fileserver/alborz/DataRelease2/sourcedata/sub-CC00379XX17/ses-120400/func/sub-CC00379XX17_ses-120400_task-rest_bold.nii.gz';
...
%%
opt.ft = 200;%75;% filter size. Determines the size of Hankel matrix formed at every voxel.
...
%% for DHCP we will work on smaller overlapping segments
...
while (vol_ind(1)<nvs)
  ...
  %% First part -  Reconstruct the time series data without scrubbing. The output of this stage is downsampled so that it has the same length as the input.
    ...
    %i = i + shift_len-1;
    %vol_ind = i:i+len_segment-1;
    opt.vol_start = opt.vol_start + shift_len-1;
    vol_ind = opt.vol_start:opt.vol_start+len_segment-1;
...
fname = strcat(data_path,'recon_',g,'_segnooverlap.nii.gz');
