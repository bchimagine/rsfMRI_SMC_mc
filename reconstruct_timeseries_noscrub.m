...
function [X,X_img,opt] = reconstruct_timeseries_noscrub(Y_img,opt)
...
BtY_np  =py.pyfuncs_sv_parallelized_ss_volumelevel.bwdMotionOperator(Y_img,interpolator,origin,spacing,direction,params,int32(n1),int32(n2),int32(nsl),int32(nv));
%% 
Zr = Zinit;
...
for i =  1: overall_maxIter
    ...
    if i==1
        Zr_np = py.pyfuncs_sv_parallelized_ss_volumelevel.solveZsubproblem(Zinit_np,rhs_zsub,params,origin,spacing,direction,interpolator,int32(n1),int32(n2),int32(nsl),int32(nv),int32(sz),beta,int32(Njobs));
    else
        Zr_np = py.pyfuncs_sv_parallelized_ss_volumelevel.solveZsubproblem(Zr_np,rhs_zsub,params,origin,spacing,direction,interpolator,int32(n1),int32(n2),int32(nsl),int32(nv),int32(sz),beta,int32(Njobs));
    end
    ...
    %% Cost computation
    thiscost = compute_cost_withbeta(Zr,Iorig,Xsamp,s,epsilon,p,mu,beta,interpolator,params,origin,spacing,direction,origin_4d,spacing_4d,direction_4d,n1,n2,nsl,nv);
    cost = [cost,thiscost];
